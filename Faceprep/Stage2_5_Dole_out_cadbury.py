# Dole Out Cadbury
# You are a teacher in reputed school. During Celebration Day you were assigned a task to distribute Cadbury such that maximum children get the chocolate. You have a box full of Cadbury with different width and height. You can only distribute largest shape Cadbury. So, if you have a Cadbury of length 10 and width 5, then you need to break Cadbury in 5x5 square and distribute to first child and then remaining 5x5 to next in queue.
...
matrix = []
for i in range(lengmin,lengmax+1):
    for j in range(widthmin,widthmax+1):
        matrix.append([i,j])
#print(matrix)
